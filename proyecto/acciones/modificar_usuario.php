<?php 

require_once "../clases/Usuario.php";
require_once "../utilidades/utils.php";
require_once "../utilidades/action_conexion.php";
session_start();

$usuario = ( new Usuario() )->getUsuarioByEmail($pdo, $_SESSION["usuario"]["email"]);
$contraseniaActual=htmlspecialchars(trim($_POST["contraseniaActual"]));
if (!$usuario || !password_verify($contraseniaActual, $usuario->getContrasenia()) ) {
    header("Location: ../index.php?seccion=modificar&error=contraseñaincorrecta");
    exit();
}

$email=htmlspecialchars(trim($_POST["email"]));
$nombre=htmlspecialchars(trim($_POST["nombre"]));
$apellido=htmlspecialchars(trim($_POST["apellido"]));
$fechaNacimiento=htmlspecialchars(trim($_POST["fechaNacimiento"]));
$contrasenia1=htmlspecialchars(trim($_POST["contrasenia1"]));
$contrasenia2=htmlspecialchars(trim($_POST["contrasenia2"]));

$email=validarLargo($email,3,100);
$nombre=validarLargo($nombre,3,30);
$apellido=validarLargo($apellido,3,30);
if(strlen($contrasenia1)>0){
$contrasenia1=validarLargo($contrasenia1,3,100);
}
if ($fechaNacimiento>date("Y-m-d", strtotime("-18 years"))){
    $fechaNacimiento="";
}

$errores= "";

if (empty($email)){
    $errores .= "<li>No se pudo validar el mail</li>";
}
if (empty($nombre)){
    $errores .= "<li>No se pudo validar el nombre</li>";
}
if (empty($apellido)){
    $errores .= "<li>No se pudo validar el apellido</li>";
}
if (empty($fechaNacimiento)){
    $errores .= "<li>Solo pueden registrarse mayores de 18 años</li>";
}

if ($contrasenia1!==$contrasenia2){
    $errores .= "<li>Las contraseñas no coinciden</li>";
}
$hasheo= true;
if (empty($contrasenia1) && empty($contrasenia2)){
    $hasheo= false;
}
if ($errores != ""){
    $_SESSION["erroresModificar"] = $errores;
    header("Location: ../index.php?seccion=modificar");
    exit();
}

try {
    $usuario->setEmail($email);
    $usuario->setNombre($nombre);
    $usuario->setApellido($apellido);
    $usuario->setFechaNacimiento($fechaNacimiento);
    if ($hasheo){
    $usuario->setContrasenia(password_hash($contrasenia1, PASSWORD_DEFAULT));
    }
    $usuario->actualizarDatosPersonales($pdo);

    $errores="sin error";
    $_SESSION["erroresModificar"] = $errores;
    $_SESSION["usuario"]["nombre"] = $usuario->getNombre();
    $_SESSION["usuario"]["email"] = $usuario->getEmail();

    header("Location: ../index.php?seccion=modificar");
    exit();
} catch (PDOException $e) {
    $errores="<li>El correo electrónico ya se encuentra registrado en el sistema.</li>";
    $_SESSION["erroresModificar"] = $errores;
    header("Location: ../index.php?seccion=modificar");
    exit();
}   
   

  ?>